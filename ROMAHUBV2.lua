...
-- ============================================
function addContentButton(text, callback, color, yPos)
    local btn = Instance.new("TextButton")
...
local function toggleFly()
    ...
    if states.fly then
        ...
        showNotification("🚀 الطيران ON (Space↑ / Shift↓)", Color3.fromRGB(0, 150, 255))
    else
...
local function showMapTeleport()
    ...
    if not MapFrame then
        ...
        MapFrame.Size = UDim2.new(0.85, 0, 0, 180)
        MapFrame.Position = UDim2.new(0.075, 0, 0, 55)
...
-- ============================================
-- 📂 إنشاء الأقسام والمحتوى (مثل Axel Hub)
-- ============================================
...
local function showTab1()
    ...
    local yPos = 5
    addContentButton("🚀 الطيران (Space↑ / Shift↓)", toggleFly, Color3.fromRGB(0, 150, 255), yPos); yPos = yPos + 42
    addContentButton("🧱 اختراق الجدران", toggleNoclip, Color3.fromRGB(150, 100, 255), yPos); yPos = yPos + 42
    addContentButton("👻 اختفاء", toggleInvisible, Color3.fromRGB(200, 100, 255), yPos)
end
...
local function showTab2()
    for _, child in pairs(ContentFrame:GetChildren()) do child:Destroy() end
    local yPos = 5
    addContentButton("⚡ تفعيل السرعة", toggleSpeed, Color3.fromRGB(0, 255, 200), yPos); yPos = yPos + 42
    addContentButton("⬆️ زيادة السرعة +10", increaseSpeed, Color3.fromRGB(50, 200, 100), yPos); yPos = yPos + 42
    addContentButton("⬇️ خفض السرعة -10", decreaseSpeed, Color3.fromRGB(200, 150, 50), yPos)
end
...
local function showTab3()
    for _, child in pairs(ContentFrame:GetChildren()) do child:Destroy() end
    local yPos = 5
    addContentButton("🌐 تيليبورت للاعب", showTeleportMenu, Color3.fromRGB(100, 150, 255), yPos); yPos = yPos + 42
    addContentButton("🗺️ تيليبورت للخريطة", showMapTeleport, Color3.fromRGB(255, 200, 50), yPos)
end
...
local function showTab4()
    for _, child in pairs(ContentFrame:GetChildren()) do child:Destroy() end
    local yPos = 5
    addContentButton("🔄 إيقاف الكل", stopAll, Color3.fromRGB(200, 50, 50), yPos)
end

-- ============================================
-- 📋 إنشاء الأزرار الجانبية (زي Axel Hub)
-- ============================================
...
-- 🔥 كود التشغيل
-- loadstring(game:HttpGet("https://raw.githubusercontent.com/fyof594-lab/MyRobloxScripts/refs/heads/main/ROMAHUBV2.lua"))()
