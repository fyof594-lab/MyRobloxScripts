...
local function createESP(plr)
    ...
    -- ✅ اختيار اللون حسب الفريق
    local color = Color3.fromRGB(255, 0, 0) -- أحمر للعدو
    if plr.Team then
        color = plr.Team.TeamColor and plr.Team.TeamColor.Color or Color3.fromRGB(255, 0, 0)
    end
    
    -- ✅ إنشاء Box Handle (إطار حول اللاعب)
    ...
    for _, plr in pairs(Players:GetPlayers()) do
        if plr ~= Player and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
            if not espObjects[plr] then
                ...
                if humanoid and espObjects[plr].healthLabel then
                    local healthPercent = math.floor((humanoid.Health / humanoid.MaxHealth) * 100)
                    espObjects[plr].healthLabel.Text = "❤️ " .. healthPercent .. "%"
                    
                    -- تغيير لون الصحة حسب النسبة
                    if healthPercent > 50 then
                        espObjects[plr].healthLabel.TextColor3 = Color3.fromRGB(0, 255, 0)
                    elseif healthPercent > 25 then
                        espObjects[plr].healthLabel.TextColor3 = Color3.fromRGB(255, 200, 0)
                    else
                        espObjects[plr].healthLabel.TextColor3 = Color3.fromRGB(255, 0, 0)
                    end
                end
            end
        end
